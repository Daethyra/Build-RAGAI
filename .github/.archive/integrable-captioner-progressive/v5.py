...
async def main():
    ...
    if env_image_folder:
        logging.info(
            f"Falling back to environment variable for IMAGE_FOLDER: {env_image_folder}"
        )
        image_folder = env_image_folder
        config["IMAGE_FOLDER"] = env_image_folder
    if env_base_name:
        logging.info(
            f"Falling back to environment variable for BASE_NAME: {env_base_name}"
        )
        base_name = env_base_name
        config["BASE_NAME"] = env_base_name
    if env_ending_caption:
        logging.info(
            f"Falling back to environment variable for ENDING_CAPTION: {env_ending_caption}"
        )
